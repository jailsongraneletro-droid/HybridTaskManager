...
const MainApp = ({ user, onLogout, onUpdateUser }: { user: User, onLogout: () => void, onUpdateUser: (u: User) => void }) => {
  ...
  const handleDragEnd = async (result: DropResult) => {
    ...
    if (startColumn === finishColumn) {
      ...
      // Update DB (Reorder)
      // For MVP: We just update the moved task. In a real app we update all affected positions.
      // Here we will just let Supabase DataService handle it or re-fetch.
      // For simplicity in MVP, we just assume the UI holds until refresh, or we update the single moved item.
