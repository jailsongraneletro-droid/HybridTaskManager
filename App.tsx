...
const MainApp = ({ user, onLogout, onUpdateUser }: { user: User, onLogout: () => void, onUpdateUser: (u: User) => void }) => {
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden font-sans relative">
      ...
      {/* Sidebar - Fix: Added relative on md: to push content instead of overlapping */}
      <aside className={`
      ...
      `}>
         <div className={`p-6 flex items-center ${isSidebarCollapsed && !isMobileMenuOpen ? 'justify-center' : 'justify-between'}`}>
            ...
            <button 
              ...
              className={`hidden md:flex p-1.5 text-slate-500 hover:text-white transition-colors ${isSidebarCollapsed ? 'absolute -right-3 top-6 bg-slate-900 rounded-full border border-slate-700 p-0.5' : ''}`}
            >
         ...
         <div className="p-4 border-t border-slate-800 space-y-2">
            <button onClick={() => setIsProfileModalOpen(true)} className={`flex items-center gap-3 p-2 rounded-xl hover:bg-slate-800 w-full text-left ${isSidebarCollapsed && !isMobileMenuOpen ? 'justify-center' : ''}`}>
               <div className="w-8 h-8 bg-indigo-500 rounded-full flex items-center justify-center font-black text-white text-xs shrink-0">{user.name.charAt(0)}</div>
