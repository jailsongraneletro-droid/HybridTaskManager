...
export const CalendarView: React.FC<CalendarViewProps> = ({ data, onEditTask, onAddTaskOnDate }) => {
  ...
  const TaskItem: React.FC<{ task: Task }> = ({ task }) => {
    ...
    return (
      <div onClick={() => onEditTask(task)} style={{ backgroundColor: `${col?.color}15`, borderLeft: `3px solid ${col?.color}` }} className="px-2 py-1 rounded-md text-[10px] font-bold text-slate-700 dark:text-slate-200 truncate cursor-pointer hover:brightness-95 transition-all shadow-sm border border-slate-200/50 dark:border-slate-700/50 mb-0.5 flex items-center gap-1.5">
        <span className="w-1 h-1 rounded-full shrink-0" style={{ backgroundColor: col?.color }}></span>
        ...
        {viewMode === 'month' && (
          <div className="grid grid-cols-7 h-full min-h-[600px] border-l border-slate-100 dark:border-slate-800">
            ...
            {monthData.map((item, idx) => {
              ...
              return (
                <div key={idx} className={`min-h-[120px] p-2 border-r border-b border-slate-100 dark:border-slate-800 transition-all group ${!item.currentMonth ? 'bg-slate-50/40 dark:bg-slate-950/20 opacity-40' : 'bg-white dark:bg-slate-900 hover:bg-slate-50/50 dark:hover:bg-slate-800/50'} ${today ? 'bg-indigo-50/10 dark:bg-indigo-900/10' : ''}`}>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <button onClick={() => onAddTaskOnDate(formatLocalDate(item.date))} className="opacity-0 group-hover:opacity-100 p-1 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/50 rounded-lg transition-all"><Plus size={12} /></button>
                  </div>
                  <div className="space-y-0.5">
                    {dateTasks.slice(0, 5).map(task => <TaskItem key={task.id} task={task} />)}
                    {dateTasks.length > 5 && <div className="text-[9px] font-black text-slate-400 mt-1">+ {dateTasks.length - 5} mais</div>}
                  </div>
                </div>
              );
            })}
          ...
          <div className="flex flex-col h-full min-w-[800px]">
            ...
            <div className="grid grid-cols-7 flex-1 min-h-[500px]">
              {weekDays.map((date, idx) => {
                ...
                return (
                  <div key={idx} className={`p-3 border-r border-slate-100 dark:border-slate-800 group hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors ${isToday(date) ? 'bg-indigo-50/5' : ''}`}>
                    <div className="space-y-2">
                      {dateTasks.map(task => (
                        <div key={task.id} onClick={() => onEditTask(task)} className="bg-white dark:bg-slate-800 p-2.5 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md cursor-pointer transition-all">
