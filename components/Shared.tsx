...
export const PriorityBadge = ({ priority }: { priority?: Priority }) => {
  if (!priority) {
      return <span className="px-1.5 py-0.5 rounded-md text-[9px] font-semibold bg-slate-100 dark:bg-slate-800 text-slate-500 border border-slate-200 dark:border-slate-700 uppercase tracking-widest">Nenhuma</span>;
  }

  return (
    <span 
      className="px-1.5 py-0.5 rounded-md text-[9px] font-semibold border uppercase tracking-widest"
      style={{
...
export const StatusBadge = ({ status, color = '#64748b' }: { status: string; color?: string }) => {
  return (
    <div 
      className="flex items-center gap-1 px-1.5 py-0.5 rounded-md text-[10px] border w-fit font-semibold"
      style={{ 
...
export const TaskAge = ({ createdAt }: { createdAt: string }) => {
  ...
  return (
    <div 
      className={`flex items-center gap-1 text-[9px] ${isOld ? 'text-red-500 font-semibold' : 'text-slate-400 dark:text-slate-500 font-medium'}`} 
    >
      <Clock size={10} />
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200 border border-slate-200 dark:border-slate-800">
        <div className="flex items-center justify-between p-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
          ...
          <button onClick={onClose} className="relative p-1.5 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition-colors bg-white/50 dark:bg-slate-800 rounded-lg group">
             ...
             <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 font-semibold text-base leading-none">×</div>
          </button>
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in duration-200 border border-slate-200">
        <div className="p-6">
          ...
          <div className="flex justify-end gap-3">
             <button onClick={onClose} className="px-4 py-2 text-[10px] font-bold uppercase tracking-widest text-slate-500 hover:bg-slate-100 rounded-xl transition-all">{t('cancel')}</button>
             <button onClick={() => { onConfirm(); onClose(); }} className="px-4 py-2 text-[10px] font-bold uppercase tracking-widest bg-red-600 text-white hover:bg-red-700 rounded-xl shadow-lg transition-all active:scale-95">{t('confirm')}</button>
          </div>
        </div>
      </div>
    </div>
